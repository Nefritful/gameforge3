...
previewBtn.addEventListener('click', async () => {
  try {
    ...
    const previewUrl = `/preview.html?owner=${encodeURIComponent(owner)}&project=${encodeURIComponent(projectName)}&scene=${encodeURIComponent(currentSceneName)}`;
    window.open(previewUrl, '_blank', 'noopener,noreferrer');
    setStatus(`Preview opened for scene: ${currentSceneName}`);
  } catch (error) {
    console.error(error);
    setStatus(`Ошибка открытия preview: ${error.message}`);
  }
});
